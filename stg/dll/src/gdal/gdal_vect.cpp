...
//	typename DataArrayBase<SequenceType>::const_reference sequence = b;
	...
	for (auto&& [y, x] : b) { // hard assumption of point order, should be reversed for 15 series
		OGRPoint pt(x, y);
		OGRLine->addPoint(&pt);
	}
...
void SetPolygonGeometryForFeature(OGRFeature* feature, SA_ConstReference<PointType> b, ValueComposition vc)
{
	...
	for (; ri != re; ++ri) // rings
	{
		...
		for (auto&& [y, x] : ring) { // hard assumption of point order, should be reversed for 15 series
			OGRPoint pt;
			pt.setX(x);
			pt.setY(y);
			OGRRing->addPoint(&pt);
