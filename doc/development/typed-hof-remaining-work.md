...
- **K11 — container-shaped types in `DefType`** **[substantial]** · *K11a-1 landed (`08ac5c8f`); K11a-1b identity plumbing landed (member attr carries sibling member-unit `vuNode`; verified flowing through `pcount`) but its def-time observability is GATED on described combining operators (`add`/`+` defers) + richer structured-param result grammar — see scope doc §5.2; K11a-3 + K11b + by-example still deferred* · **scoped: `k11-container-types-scope.md`**
