...
- **for_each / meta-group full typed-container expansion** **[substantial]** · *DONE for `for_each_*` (verified 2026-07-28); `loop` & co still defer*
  **The "needs the K11 gate" sequencing note was STALE** — the §12.7 impedance tranche built the container `DefType` it needed instead of waiting, and `TryMetaContainerProcessing` already branches before the `dont_cache_result` deferral (the doc's second gate). Verified empirically 2026-07-28: a `for_each_nedv` result inside a function body IS a fully typed container — its members carry both DOMAIN and VALUES, so the walker reports, at the DEFINITION, (a) a member declared over the wrong domain, (b) a member declared with the wrong values class, (c) a reference to a name the array does not generate ("generates member(s) m1, m2; 'm3' is not among them"), and (d) the ∀ case: with the domain argument a FORMAL unit parameter the member's type rides that formal's unifier node, so declaring it over a different formal is a rigid-rigid conflict. Pinned by `fn_test_fe_neg1` (member set), `_neg4` (values class), `_neg8` (domain, under ∀), `_neg2`/`_neg7` (arity + invalid spec), `_neg5`/`_neg6` (the defer guards: open name array, deep path).
  **Genuinely remaining:** `MemberKind::Untyped` variants (`for_each_ne` — no domain/values in the suffix, so a member's type could only come from evaluating its EXPRESSION STRING: out of scope by design, that is the untyped string metaprogramming the design deliberately keeps outside) and `TemplateCopy` members (typing them would need definition-time template instantiation). `loop` and other meta groups still default `DescribeMetaSignature=false` and defer wholesale.
  *Sources: op-sig §12.7 (1347-1417) — its "two extra gates" sequencing paragraph is superseded.*

- **Tier-2 meta `SelectMetaOperator` composite-result typing** **[substantial]** · *CLOSED by ruling (Maarten, 2026-07-29)*
  `select_with_attr_by_cond`, `select_with_org_rel_with_attr_by_cond`, `select_with_attr_by_org_rel` (+uintN), `collect_attr_by_{cond,org_rel}`. **Ruling: per-instantiation evaluation of these meta selections on formal arguments is ACCEPTED, not a gap** — the attributes are unknown to the function definition and that is fine. These operators are borderline deprecation-candidates *because* of such troubles; as long as they remain harmless (defer cleanly, no false definition-time errors) no typing effort is warranted. Current sound behavior pinned by `fn_test_selmeta{,_neg}`.
  *Sources: op-sig §12.9 Tier 2 (1527-1560); design §11 ruling note.*
...
- **§9 `SigUnitChecker` drift-verifier** **[moderate]** · *both defenses landed + armed; two v1 coverage gaps remain*
  ...
  **v2 (2026-07-29) closed both v1 gaps:** (a) `rec.resultMembers` ARE now replayed against the actual result's
  sub-items (absence = conditional member, no claim; presence = the described vars' units are collected like a
  position; liveness canary confirmed `connect_info`'s 7 members + `discrete_alloc`'s landuse/bid_price resolve);
  (b) EVERY collected values-unit is class-checked, covering a `RepeatArgs`/variadic tail's later positions.
  Still out: `resultMemberSets` (their names live in argument DATA, which the debug checker must not read), and —
  by the 2026-07-29 ruling — metrics stay log-only permanently. NOTE: a described family NOT exercised by the
  battery (e.g. some geo/aggregation members, `rlookup`) could surface its FIRST assert in a tst-Debug or
  GUI-Debug run — that is the intended drift signal, to be fixed at the description.
...
- **`unit_creator_spec` + `uc_*` registration factories** **[moderate]** · *CLOSED by the metric ruling (2026-07-29)*
  Pair a `UnitDerivationKind` with a `UnitCreatorPtr` so K9/K10 values-unit derivation is consumable at definition time. Was "only useful once metric checking un-defers" — and metric checking now stays instantiation-time by ruling, so this is moot.
...
- **Declared metric constraints on function parameters (un-defer K10)** **[substantial]** · *CLOSED by ruling (Maarten, 2026-07-29)*
  **Metric compatibility stays an instantiation-time concern, permanently, by design** — no declared-metric syntax, no definition-time metric unification, no "fuss at definition time". The technical door stays on record ("vacuous not intractable": a decidable free abelian group, the op-sig `MetricProduct/Quotient/Power` vocabulary already carries the relations) but is deliberately not walked through. The §9 SigUnitChecker's metric checks stay log-only for the same reason; this also closes what was gated on it (the `unit_creator_spec` factories; WP4.4 beyond cosmetics).
  *Sources: design §11 (2288-2304, incl. the ruling note); op-sig §7 K10 (570), §15 Q4 (1629-1634), §20.1.*
