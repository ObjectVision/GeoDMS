...
| [#1105](https://github.com/ObjectVision/GeoDMS/issues/1105) geodms.pyd Python ABI mismatch across .m/.c installers | Windows implementation prepared with a shared CPython 3.9/3.12/3.13/3.14 ABI matrix: each module is ABI-tagged, packaging verifies every module and runtime import, Python runtimes are not bundled, and import refuses an already-loaded foreign GDAL. Interactive `.m`/`.c` release validation and the wider multi-GDAL policy remain open. |
