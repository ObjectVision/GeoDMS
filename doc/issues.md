...
Snapshot of the 45 open issues at https://github.com/ObjectVision/GeoDMS/issues, taken 2026-07-03,
...
| [#872](https://github.com/ObjectVision/GeoDMS/issues/872) Export dialog reports success on error | **Closed (2026-07-03, `refactor_ownership`):** `exportImpl` returns success; fail reasons, exceptions and the no-exportable-data case pop a modal "Export failed" box, and the dialog resets for a retry instead of claiming "Export ready". |
| [#823](https://github.com/ObjectVision/GeoDMS/issues/823) Native CSV export button | Defaults (folder, separator, null handling); overlaps #411/#1124. |
| [#411](https://github.com/ObjectVision/GeoDMS/issues/411) Table "save to file" opens Export Primary Data dialog | **Closed (2026-07-03, `refactor_ownership`):** floppy builds a `Desktops/Default/ViewData` config table referencing the thematic attributes of all DataItemColumns and opens the Export Primary Data dialog on it. Also added an XML (*.xml) export format (GML driver, works without geometry) plus two storage fixes: GML classified non-updatable, and the dialog holds interest in all storable items so layers are written in one session. Polish left: folder suggestion, ViewData container accumulation, group-by column test. |
| [#1124](https://github.com/ObjectVision/GeoDMS/issues/1124) Floppy should also trigger ExportSettings xml write | Same export-flow cluster as #823/#411. |
| [#630](https://github.com/ObjectVision/GeoDMS/issues/630) Rethink save-to-bitmap button | Suggestion (dialog: tiled bitmaps vs single png) is in the issue; confirm and build. |
...
- **Export-flow cluster** (#823, #411, #1124, #630, #872, #973, #711): one shared design decision about
  what the floppy/export dialog should do resolves six issues in one sweep. #411 is done and provides
  the pattern: route table exports through the Export Primary Data dialog via a constructed
  `Desktops/Default/ViewData` config table.
