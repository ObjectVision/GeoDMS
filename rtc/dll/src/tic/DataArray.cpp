		...
		parallel_tileloop_if_separable<V>(tn, [shadowTilePtr, trd, &shadowData, shadowRangeBase = range.first, minValuePtr](tile_id t)
			{
				...
				assert(Cardinality(tileRange) == tileData.size());
				SizeT shadowDataSize = shadowTilePtr->data_size();
				SizeT tileDataSize = tileData.get_sa().data_size();
				SizeT offset = tileData.get_sa().data_begin() - minValuePtr;
				assert(!tileDataSize || offset <= shadowDataSize);
				assert(!tileDataSize || offset + tileDataSize <= shadowDataSize);
...
{
	return GetTokenID_st(myArrayPrintF<100>("DataItem<{}>", ValueWrap<T>::GetStaticClass()->GetNameID().c_str_st()));
}
...
auto CreateAbstrHeapTileFunctor(const AbstrDataItem* adi, SharedPtr<const SharedObj> abstrValuesRangeData, const bool mustClear MG_DEBUG_ALLOCATOR_SRC_ARG) -> std::unique_ptr<AbstrDataObject>
{
	...
	if (currTRD->GetNrTiles() > 1 && !adi->IsCacheItem())
		reportF(MsgCategory::other, SeverityTypeID::ST_MinorTrace, "CreateAbstrHeapTileFunctor(attribute<{}> {}({} tiles))", avu->GetValueType()->GetNameID(), adi->GetFullName().c_str(), currTRD->GetNrTiles());
