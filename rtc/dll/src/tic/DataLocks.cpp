...
DataWriteLock::DataWriteLock(AbstrDataItem* adi, dms_rw_mode rwm, const SharedObj* abstrValuesRangeData) // was lockTile 
{
	...
	if (configItem && !configItem->IsCacheItem())
	{
		if (auto sp = configItem->GetCurrStorageParent(true))
		{
			...
			{
				// #1179: an IntegrityCheck on a stored sub-item is refused in the MMD arm of
				// TreeItem::PrepareDataUsage, on the meta thread before this production starts.
				// It was refused here until #1275: this lock runs on the producing thread, where a
				// stored property may not be read.
