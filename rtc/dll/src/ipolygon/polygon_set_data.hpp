...
//REMOVE #include "polygon_45_set_data.hpp"
//REMOVE #include "polygon_45_set_concept.hpp"
...
namespace boost { namespace polygon {
  ...
  public:
	...
	struct clean_resources; // forward decl
	struct convolve_resources; // forward decl
    ...
    template <typename ltype, typename rtype, int op_type>
    inline polygon_set_data& operator_is(const polygon_set_view<ltype, rtype, op_type>& geometry, clean_resources& resources) 
	{
      (*this) = geometry.value(resources.executeResources);
      dirty_ = false;
      unsorted_ = false;
      return *this;
    }
    ...
    inline polygon_set_data& operator_is(const geometry_object& geometry, clean_resources& resources)
	{
      data_.clear();
      insert(geometry);
      return *this;
    ...
    inline void get(output_container& output, clean_resources& r) const {
      get_dispatch(output, r, typename geometry_concept<typename output_container::value_type>::type());
    }
	...
	void move(point_type p)
	{
		for (auto& he : data_) {
			convolve(he.first.first, p);
			convolve(he.first.second, p);
		}
	}
    ...
    inline std::size_t size(clean_resources& r) const { clean(r); return data_.size(); }
	...
	void clean(clean_resources& r) const;
    ...
    inline polygon_set_data&
    resize(coordinate_type resizing, bool corner_fill_arc /*= false*/, unsigned int num_circle_segments /*=0*/, convolve_resources& resources);
    ...
    polygon_set_data&
    bloat(typename coordinate_traits<coordinate_type>::unsigned_area_type distance, clean_resources& cleanResources) {
      std::list<polygon_with_holes_data<coordinate_type> > polys;
      get(polys, cleanResources);
      clear();
    ...
    polygon_set_data&
    shrink(typename coordinate_traits<coordinate_type>::unsigned_area_type distance, clean_resources& cleanResources) {
      std::list<polygon_with_holes_data<coordinate_type> > polys;
      get(polys, cleanResources);
      clear();
      for(typename std::list<polygon_with_holes_data<coordinate_type> >::iterator itr = polys.begin();
  ...
  private:
    ...
    mutable bool is_45_; // TODO: rewrite assuming is_45_ is always false
  ...
  private:
    ...
    void get_dispatch(output_container& output, clean_resources& r, polygon_with_holes_concept tag) const {
      get_fracture(output, false, r, tag);
    }
    template <typename output_container, typename concept_type>

    void get_fracture(output_container& container, bool fracture_holes, clean_resources& r, concept_type ) const {
		MG_DEBUGCODE(
			static int c = 0;
			c++;
		)
...
namespace boost { namespace polygon {
  ...
  public:
	using extract_resources = typename ce::execute_resources;
	struct insert_resources {
		typename polygon_set_data<coordinate_type>::clean_resources cleanResources;
		polygon_set_data<coordinate_type> tmp_psd;
	};
    ...
    inline unsigned int insert(const polygon_set_data<coordinate_type>& ps, typename polygon_set_data<coordinate_type>::clean_resources& r) {
      ps.clean(r);
      ce_.populateTouchSetData(ps.begin(), ps.end(), nodeCount_);
      return nodeCount_++;
    }
    template <class GeoObjT>
    inline unsigned int insert(const GeoObjT& geoObj, insert_resources& r)
	{
      r.tmp_psd.clear();
	  r.tmp_psd.insert(geoObj);
      return insert(r.tmp_psd, r.cleanResources);
    }
    ...
    inline void extract(GraphT& graph, extract_resources& resources) {
      ce_.execute(graph, resources);
    }
  };
