...
namespace boost { namespace polygon{
  ...
  struct polygon_set_traits {
	...
	static inline bool clean(const T&, clean_resources& cleanResources) { return false; }
  ...
  struct polygon_set_mutable_traits<std::list<T> > {
	using coordinate_type = typename polygon_set_traits<std::list<T> >::coordinate_type;
	using polygon_set_data_type = polygon_set_data<coordinate_type>;
	using clean_resources = typename polygon_set_data_type::clean_resources;
    template <typename input_iterator_type>
    static inline void set(std::list<T>& polygon_set, input_iterator_type input_begin, input_iterator_type input_end, clean_resources& cleanResources) 
	{
      polygon_set.clear();
      polygon_set_data_type ps;
      ps.reserve(std::distance(input_begin, input_end));
      ps.insert(input_begin, input_end);
      ps.get(polygon_set, cleanResources);
    }
	static inline void set(std::list<T>& polygon_set, const polygon_set_data_type& ps, clean_resources& cleanResources)
	{
		polygon_set.clear();
		ps.get(polygon_set, cleanResources);
	}
  ...
  struct polygon_set_mutable_traits<std::vector<T> > {
	  using coordinate_type = typename polygon_set_traits<std::vector<T> >::coordinate_type;
	  using polygon_set_data_type = polygon_set_data<coordinate_type>;
	  using clean_resources = typename polygon_set_data_type::clean_resources;

	  template <typename input_iterator_type>
	static inline void set(std::vector<T>& polygon_set, input_iterator_type input_begin, input_iterator_type input_end, clean_resources& cleanResources)
		{
		  polygon_set.clear();
	...
	static inline void set(std::vector<T>& polygon_set, const polygon_set_data_type& ps, clean_resources& cleanResources)
	{
		polygon_set.clear();
		ps.get(polygon_set, cleanResources);
	}
  ...
  struct polygon_set_mutable_traits<polygon_set_data<T> > {
	  ...
	  template <typename input_iterator_type>
    static inline void set(polygon_set_data<T>& polygon_set, input_iterator_type input_begin, input_iterator_type input_end, clean_resources& cleanResources)
	{
      polygon_set.set(input_begin, input_end);
    }
	static inline void set(polygon_set_data<T>& polygon_set, const polygon_set_data_type& input, clean_resources& cleanResources)
	{
		input.clean(cleanResources);
		polygon_set = input;
	}
  };
  template <typename T>
  struct polygon_set_traits<polygon_set_data<T> > {
	...
	typedef typename polygon_set_data<T>::clean_resources clean_resources;
//REMOVE	typedef typename arbitrary_boolean_op<T>::execute_resources execute_resources;
    ...
    static inline bool clean(const polygon_set_data<T>& polygon_set, clean_resources& r) { polygon_set.clean(r); return true; }
