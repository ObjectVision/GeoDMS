...
namespace boolean_op {
  ...
  struct applyBooleanBinaryOp_resources {
  ...
  inline void applyBooleanBinaryOp(std::vector<std::pair<Unit, std::pair<Unit, int> > >& output,
                                   ...
                                   T defaultCount
	, applyBooleanBinaryOp_resources<Unit>& resources) {
    ...
    while(itr1 != itr1_end || itr2 != itr2_end) {
      ...
      if(curPosition != prevPosition && count != defaultCount) {
		...
		auto& container = resources.container; container.clear();
        boolean.processInterval(resources.container, ivl, count);
        for(std::size_t i = 0; i < resources.container.size(); ++i) {
