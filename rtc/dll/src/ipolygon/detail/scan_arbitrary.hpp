...
//     }
	...
	struct compute_histogram_in_y_resources {
    ...
    static inline void compute_histogram_in_y(iT begin, iT end, std::size_t size, std::vector<std::pair<Unit, std::pair<std::size_t, std::size_t> > >& histogram
		, compute_histogram_in_y_resources& r) {
	  auto& ends = r.ends; ends.clear(); ends.reserve(size * 2);
	...
	struct validate_point_scan_resources {
		std::vector<Point> pts;
		std::vector<std::pair<half_edge, segment_id> > data;
	};

	struct validate_scan_divide_and_conquer_resources {
		std::vector<std::pair<Unit, std::pair<std::size_t, std::size_t> > > histogram;
		compute_histogram_in_y_resources chyr;
		std::vector<Unit> y_cuts;
		std::map<Unit, std::vector<std::pair<half_edge, segment_id> > > bins;
		//std::unique_ptr < validate_scan_edge_resources > vesPtr;
		validate_point_scan_resources vpsr;
	};

	struct validate_scan_edge_resources {
		std::vector<std::set<Point> > intersection_points;
		validate_scan_divide_and_conquer_resources vsdacr;
	};

	template <typename property_type>
	struct validate_scan_property_resources {
		...
		validate_scan_edge_resources vser;
	};

    template <typename iT>
    static inline void validate_scan_divide_and_conquer(std::vector<std::set<Point> >& intersection_points, iT begin, iT end
			,	validate_scan_divide_and_conquer_resources& r) 
	{
	  auto& histogram = r.histogram; histogram.clear();
      compute_histogram_in_y(begin, end, std::distance(begin, end), histogram, r.chyr);

	  auto& y_cuts = r.y_cuts; y_cuts.clear();
      compute_y_cuts(y_cuts, histogram.begin(), histogram.end(), std::distance(begin, end));

	  auto& bins = r.bins; bins.clear();

      bins[histogram.front().first].clear();
      for(typename std::vector<Unit>::iterator itr = y_cuts.begin(); itr != y_cuts.end(); ++itr) {
        bins[*itr].clear();
      }

      for(iT itr = begin; itr != end; ++itr) {
        auto lb = bins.lower_bound((std::min)((*itr).first.first.y(), (*itr).first.second.y()));
        if(lb != bins.begin())
          --lb;
        auto ub = bins.upper_bound((std::max)((*itr).first.first.y(), (*itr).first.second.y()));
        for( ; lb != ub; ++lb) {
          (*lb).second.push_back(*itr);
        }
      }
	  validate_scan(intersection_points, bins[histogram.front().first].begin(), bins[histogram.front().first].end(), r.vpsr);
      for(typename std::vector<Unit>::iterator itr = y_cuts.begin(); itr != y_cuts.end(); ++itr) {
        validate_scan(intersection_points, bins[*itr].begin(), bins[*itr].end(), *itr, r.vpsr);
      }
    }

    template <typename iT>
    static inline void validate_scan(std::vector<std::set<Point> >& intersection_points, iT begin, iT end, validate_point_scan_resources& r) 
	{
      validate_scan(intersection_points, begin, end, (std::numeric_limits<Unit>::min)(), r);
    }

    //quadratic algorithm to do same work as optimal scan for cross checking

	template <typename iT>
    static inline void validate_scan(std::vector<std::set<Point> >& intersection_points, iT begin, iT end, Unit min_y, validate_point_scan_resources& r) {
	  auto& pts = r.pts;  pts.clear();
	  auto& data = r.data; data.clear(); data.insert(data.begin(), begin, end);
    ...
    static inline void validate_scan(std::vector<std::pair<half_edge, std::pair<property_type, int> > >& output_segments,
                                     iT begin, iT end
		, validate_scan_property_resources<property_type>& r) {

      r.input_properties.clear();
	  r.input_segments.clear();
	  r.intermediate_segments.clear();

      int index = 0;
      for( ; begin != end; ++begin) {
        r.input_properties.push_back((*begin).second);
		r.input_segments.push_back(std::make_pair((*begin).first, index++));
      }
      validate_scan(r.intermediate_segments, r.input_segments.begin(), r.input_segments.end(), r.vser);
      for(std::size_t i = 0; i < r.intermediate_segments.size(); ++i) {
        output_segments.push_back(std::make_pair(r.intermediate_segments[i].first,
                                                 r.input_properties[r.intermediate_segments[i].second]));
        less_point lp;
        if(lp(output_segments.back().first.first, output_segments.back().first.second) !=
           lp(r.input_segments[r.intermediate_segments[i].second].first.first,
			   r.input_segments[r.intermediate_segments[i].second].first.second)) {
          //edge changed orientation, invert count on edge
          output_segments.back().second.second *= -1;
        }
        if(!scanline_base<Unit>::is_vertical(r.input_segments[r.intermediate_segments[i].second].first) &&
    ...
    template <typename iT>
    static inline void validate_scan(std::vector<std::pair<half_edge, int> >& output_segments, iT begin, iT end, validate_scan_edge_resources& r) 
	{
	  auto& intersection_points = r.intersection_points; intersection_points.clear();
	  intersection_points.resize(std::distance(begin, end));
      validate_scan_divide_and_conquer(intersection_points, begin, end, r.vsdacr);
...
#pragma warning (pop)
  ...
  public:
    ...
    void merge(result_type& result, property_merge_data& tmp_pmd) {
      if(pmd.empty()) return;
      //intersect data
      tmp_pmd.clear();
...
#pragma warning (pop)
  ...
  public:
	...
	inline void executeArbitrary(result_type& result, iT1 b1, iT1 e1, iT2 b2, iT2 e2, int op, execute_resources& r) {
		//intersect data
		insert(b1, e1, 0);
		insert(b2, e2, 1);
		r.tmp_pmd.clear();
		...
		if (pmd.empty())
			return;
		line_intersection<Unit>::validate_scan(r.tmp_pmd, pmd.begin(), pmd.end(), r.vspr);
		pmd.swap(r.tmp_pmd);
		sort_property_merge_data();
	...
	template <int op, typename result_type, typename iT1, typename iT2>
    inline void execute(result_type& result, iT1 b1, iT1 e1, iT2 b2, iT2 e2, execute_resources& r) {
      //intersect data
      insert(b1, e1, 0);
      insert(b2, e2, 1);
	  r.tmp_pmd.clear();
      ...
      sort_property_merge_data();
      scanline<Unit, property_type, std::vector<property_type> > sl;
      boolean_output_functor<result_type, std::vector<property_type>, op> bof;
      sl.scan(result, bof, pmd.begin(), pmd.end());
    }

	template <typename result_type, typename iT1>
	inline void execute_clean(result_type& result, iT1 b1, iT1 e1, execute_resources& r) {
		//intersect data
		insert(b1, e1, 0);
		if (pmd.empty())
			return;
		...
		sort_property_merge_data();
		scanline<Unit, property_type, std::vector<property_type> > sl;
		boolean_output_functor<result_type, std::vector<property_type>, BOOLEAN_OR> bof;
		sl.scan(result, bof, pmd.begin(), pmd.end());
	}
  ...
  private:
    template <typename iT>
    void insert(iT b, iT e, property_type id) {
      for(;
          b != e; ++b) {
  ...
  public:
	...
	struct execute_resources {
		typename line_intersection<Unit>::validate_scan_property_resources<property_type> vspr;
	};

    template <typename result_type>
    inline void execute(result_type& result, execute_resources& resources) {
      //intersect data
      property_merge_data tmp_pmd;
      line_intersection<Unit>::validate_scan(tmp_pmd, pmd.begin(), pmd.end(), resources.vspr);
      pmd.swap(tmp_pmd);
      sort_property_merge_data();
      scanline<Unit, property_type, std::vector<property_type> > sl;
      std::pair<std::pair<Unit, std::map<point_data<Unit>, std::set<property_type> > >, result_type*> output
