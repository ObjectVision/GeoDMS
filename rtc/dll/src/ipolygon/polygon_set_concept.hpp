...
namespace boost { namespace polygon{
  ...
  typename enable_if< typename is_polygon_set_type<polygon_set_type>::type,
                       bool>::type
  clean(const polygon_set_type& polygon_set, typename polygon_set_data<typename polygon_set_traits<polygon_set_type>::coordinate_type>::clean_resources& r)
  {
    return polygon_set_traits<polygon_set_type>::clean(polygon_set, r);
  }
  ...
  typename enable_if< typename gtl_and<
	...
	assign(polygon_set_type_1& lvalue, const polygon_set_type_2& rvalue, 
		typename polygon_set_data<typename polygon_set_traits<polygon_set_type_2>::coordinate_type>::clean_resources& r) 
	{
    if(clean(rvalue, r))
		polygon_set_mutable_traits<polygon_set_type_1>::set(lvalue, 
			begin_polygon_set_data(rvalue), end_polygon_set_data(rvalue)
		, r);
    else {
		using polygon_set_data_type = polygon_set_data<typename polygon_set_traits<polygon_set_type_2>::coordinate_type>;
		polygon_set_data_type ps;
		ps.insert(begin_polygon_set_data(rvalue), end_polygon_set_data(rvalue));
		ps.clean(r);
		polygon_set_mutable_traits<polygon_set_type_1>::set(lvalue, ps.begin(), ps.end(), r);
    }
    return lvalue;
  }
  ...
  empty(const polygon_set_type& polygon_set,
	  typename polygon_set_data<typename polygon_set_traits<polygon_set_type>::coordinate_type>::clean_resources& r) {
    if(clean(polygon_set, r))
		return begin_polygon_set_data(polygon_set) == end_polygon_set_data(polygon_set);
    polygon_set_data<typename polygon_set_traits<polygon_set_type>::coordinate_type> ps;
    assign(ps, polygon_set, r);
    ps.clean(r);
    return ps.empty();
  }
  ...
  typename enable_if< typename gtl_and<
    ...
    typename is_mutable_rectangle_concept<typename geometry_concept<rectangle_type>::type>::type>::type,
                       bool>::type
  extents(rectangle_type& extents_rectangle, const polygon_set_type& polygon_set) 
  {
    clean(polygon_set);
    polygon_set_data<typename polygon_set_traits<polygon_set_type>::coordinate_type> ps;
    assign(ps, polygon_set);
  ...
  resize(polygon_set_type& polygon_set, coord_type resizing,
	  bool corner_fill_arcs /*= false*/, int num_circle_segments /*= 0*/,
	  typename polygon_set_data< typename polygon_set_traits<polygon_set_type>::coordinate_type >::convolve_resources& r)
  {
    typedef typename polygon_set_traits<polygon_set_type>::coordinate_type Unit;
    clean(polygon_set, r.cleanResources);
    polygon_set_data<Unit> ps;
    assign(ps, polygon_set, r.cleanResources);
    ps.resize(resizing, corner_fill_arcs,num_circle_segments, r);
    assign(polygon_set, ps, r.cleanResources);
    return polygon_set;
  }
  ...
  keep(polygon_set_type& polygon_set,
	   ...
	   typename polygon_set_data< typename polygon_set_traits<polygon_set_type>::coordinate_type>::clean_resources& resources
  ) {
    typedef typename polygon_set_traits<polygon_set_type>::coordinate_type Unit;
    typedef typename coordinate_traits<Unit>::unsigned_area_type uat;
    std::list<polygon_with_holes_data<Unit> > polys;
    assign(polys, polygon_set, resources);
    ...
    assign(polygon_set, polys, resources);
    return polygon_set;
  }
