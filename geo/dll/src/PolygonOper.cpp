...
// dms_intersect, dms_union, dms_xor and dms_difference (issue #1214): polygon overlay with a
// fault-tolerant sweep, the home-brewed fifth backend next to bp_ / bg_ / cgal_ / geos_.
//
// ==== What it promises ================================================================
//
// The four Boolean operations on polygon attributes, element by element (both domains equal, or
// one of them Void), for all six point types. Unlike the library backends it does not require
// valid input: rings may self-intersect, self-touch, overlap or touch each other, be nearly but
// not quite closed, or carry near-coincident vertices. The semantics that make this well defined
// is the EVEN-ODD RULE, stated here explicitly as the GeoDMS meaning of a polygon sequence:
//
//     a point is inside the polygon when a ray from it to infinity crosses the boundary an odd
//     number of times.
//
// Ring orientation therefore plays no role on input (the result is still written with clockwise
// shells and counter-clockwise holes, the GeoDMS convention that area() is positive for). The
// zero-width corridors that connect the rings of a multi-polygon in the sequence layout are
// walked twice, in opposite directions, and cancel modulo 2, so the sequence as a whole is one
// closed walk, and that is exactly how it is read: consecutive points are segments, the final
// point is connected back to the first, and ring detection is not needed at all.
//
// ==== Tolerance =========================================================================
//
// Intersections are snapped to a grid, and every vertex of the input is snapped to the same grid
// first. That is the tolerance: two vertices closer than a grid cell become one, a sliver thinner
// than a cell collapses (and, being a polygon operator, a collapsed line or point is simply not in
// the result: polygon in, polygon out). The grid is
//
//   - for the two-argument form on integer coordinates: the integer grid itself, so a vertex is
//     never moved and the result is exact in the sense that all its vertices are integer points;
//   - for the two-argument form on float coordinates: a power of two derived per element pair,
//     the coarser of the unit in the last place of the largest coordinate magnitude and
//     (joint extent) / 2^36. That is far below any real precision of geographic data and it keeps
//     the exact arithmetic below within 128 bits;
//   - for the three-argument form, dms_xxx(a, b, grid): the given grid size in coordinate units,
//     a whole number for integer coordinates. This is where a modeller asks for real tolerance.
//
// The answers to the design questions on #1214: the tolerance is an operator argument with a
// derived default, not a configuration setting; the result is always a valid even-odd polygon,
// and the snapping is reported in the trace (nodes snapped, extra noding rounds) rather than
// through a diagnostic attribute, which would need a second result item; the operator sits
// beside the four library backends and their clean-up pre-passes, it does not replace them.
//
// ==== The algorithm =====================================================================
//
// 1. Quantize both sequences to the grid; internal coordinates are Int64 pixel centres in
//    [0, 2^36], relative to the grid line at or below the joint bounding box, so that the lattice
//    is the same for every pair with the same cell. Build the closed walks as segments tagged
//    with their operand (A or B).
// 2. Node the segments by ITERATED SNAP ROUNDING until a fixed point:
//      hot pixels := every segment endpoint plus the pixel of every proper crossing (the crossing
//                    computed exactly with 128-bit rationals, rounded half up);
//      every segment is replaced by the chain through the centres of all hot pixels it meets
//      (pixel = half-open unit square, so pixels partition the plane and the chain has no ties);
//      identical fragments merge and their per-operand multiplicities are kept modulo 2.
//    A pass that splits nothing is the fixed point: no proper crossings, no vertex in the interior
//    of a fragment, no partial overlaps. Snapping can create new incidences (a moved vertex lands
//    on a third segment), which is why the passes are iterated; in practice two suffice.
// 3. A plane sweep over the fragments, lexicographic in (x, y) with the usual symbolic tilt so
//    that vertical fragments order above everything else through their lower endpoint, keeps the
//    active fragments in a std::set. The parity of A and of B in the face just below a fragment
//    is the parity above its predecessor in the set, so both face parities of every fragment are
//    known after one sweep, whatever the number of components, without any point location.
// 4. A fragment is a result boundary when the Boolean function of (insideA, insideB) differs on
//    its two sides; it is directed so that the result lies on its right.
// 5. The directed edges are chained into rings by turning into the interior at every vertex, and
//    a walk that returns to a vertex it has visited pinches the loop off as its own ring, so every
//    ring is simple. Clockwise rings (positive GeoDMS area) are shells, the others holes.
// 6. A second sweep, over the ring edges, finds the shell of every hole: the active edge directly
//    below a hole's lexicographically first vertex bounds the polygon interior around that vertex,
//    so it belongs to the hole's shell or to a sibling hole processed earlier.
// 7. The rings are written in the GeoDMS multi-polygon layout, the one geos_write_mp and
//    bp_assign_mp produce: shell, holes, back-track points, and the polygons themselves connected
//    by their back-track points. Shells and holes are ordered by their first vertex and start at
//    it, and a vertex between two collinear boundary edges (a noding artefact) is left out, so the
//    result does not depend on the operand order or on the vertex order of the input: A + B == B + A
//    holds exactly, and A * A is A with its collinear vertices dropped.
//
// ==== Exact arithmetic ==================================================================
//
// All predicates (orientation, fraction comparison, the segment/pixel test) and the crossing
// pixel are computed exactly. With internal coordinates below 2^36, differences are below 2^37,
// a cross product below 2^75, and the numerator of a crossing coordinate, p.x * den + r.x * num,
// below 2^114: comfortably within boost::multiprecision::int128_t. Orientation takes an Int64
// fast path when all differences are below 2^31, which is nearly always.
//
// ==== The dms_ family, and what is deferred ==============================================
//
// dms_ is the prefix of the operators that GeoDMS implements itself, next to the four library
// prefixes. These four are the first; the rest of the family, all to be built on this engine:
//   - dms_union_polygon (dissolve, optionally partitioned): the union of many operands is the
//     even-odd overlay of all their walks at once, with one parity bit per partition;
//   - dms_polygon: the unary clean-up, the union of an operand with itself;
//   - dms_polygon_connectivity and dms_overlay_polygon: PolygonOverlayOperator in BoostPolygon.cpp
//     dispatches on geometry_library and needs a fifth branch that calls DmsOverlayEngine, a
//     one-place change once that TU is free;
//   - dms_minkowski_sum and dms_minkowski_difference: the convex cells of minkowski.h, dissolved
//     with the union above instead of with a library.
//   - The infix operators (* + - ^) keep their bp_ (integer) and geos_ (float) bindings.
//   - The hot-pixel query for a long diagonal segment visits every hot pixel in its bounding box
//     before the exact test filters them; a segment-walk over the quadtree would do better on
//     inputs where a few very long segments cross a dense polygon.
//   - Wiki: the polygon operators page and one page per operator (geodms-wiki).
...
struct DmsOverlayEngine
{
	...
	DmsOverlayEngine(BoolOp op, CharPtr operName, Float64 explicitGrid)
	...
	{
		if (!IsDefined(a) || !IsDefined(b) || !SetFrame(a, b))
		{
			++m_NrUndefined;
		...
		for (SizeT i = 0; i != n; ++i)
		{
			bool inRight = IsInside(m_Op, m_Parity[i].rightMask);
			bool inLeft  = IsInside(m_Op, m_Parity[i].rightMask ^ m_Parity[i].mask);
			if (inRight == inLeft)
				continue;
			if (inRight)
...
private:
	...
	bool SetFrame(SA_ConstReference<P> a, SA_ConstReference<P> b)
	{
		...
		if (!any)
			minX = minY = maxX = maxY = 0; // empty operands: any frame will do
		...
		if constexpr (is_float)
		{
			Float64 extent = Max<Float64>(maxX - minX, maxY - minY);
			...
			{
				// the coarser of: 2^36 cells over the joint extent, and the unit in the last place
				// of the largest coordinate magnitude (below which the input has no information)
				...
				m_Cell = e == std::numeric_limits<int>::min() ? 1.0 : std::ldexp(1.0, e);
			}
	...
	void BuildWalk(SA_ConstReference<P> seq, UInt8 mask)
	{
