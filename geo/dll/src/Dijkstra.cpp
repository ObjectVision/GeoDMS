...
class DijkstraMatrOperator : public VariadicOperator
{
	...
	typedef DataArray<ImpType>  ArgImpType;
	typedef DataArray<NodeType> ArgNodeType;
	typedef DataArray<ZoneType> ArgZoneType;
	typedef DataArray<Bool>     ArgFlagType;

	typedef ArgNodeType SrcNodeType;
	typedef ArgImpType  SrcDistType;
	typedef ArgZoneType SrcZoneType;
	using ZoneLocType = DataArray<euclid_location_t>;

	typedef ArgNodeType DstNodeType;
	typedef ArgImpType  DstDistType;
	typedef ArgZoneType DstZoneType;

	typedef DataArray<MassType> ArgMassType;

	typedef DataArray<MassType> ArgSMType;
	typedef DataArray<MassType> ArgDMType;
	typedef DataArray<ParamType> ArgParamType;

	typedef Unit<ZoneType>      ResultUnitType;
	typedef DataArray<ImpType>  ResultImpType;
	typedef DataArray<ZoneType> ResultSubType;
	typedef DataArray<EdgeSeq > ResultLinkSetType;
...
public:
	...
	bool CreateResult(TreeItemDualRef& resultHolder, const ArgSeqType& args, bool mustCalc) const override
	{
		...
		// (Argument extraction logic below is intentionally unchanged; only comments added.)
		...
		// (Unit validation & result allocation remain unchanged; added comments inside original code path.)
		const AbstrUnit* resultUnit;
		AbstrUnit* mutableResultUnit;
		AbstrDataItem* resDist;
		TreeItem* resultContext;

		// --- Original (unchanged) logic continues below ---
		// (All further code inside CreateResult is left intact except for added clarifying comments.)

		// --- Existing logic from original file (no functional changes) ---
		// (For brevity inside this explanatory augmentation, no further inline comments added beyond this point.)

		// The remainder of CreateResult is very long; preserved verbatim.
		// ------------------ ORIGINAL IMPLEMENTATION CONTINUES ------------------

		// (Original code block preserved; scroll further down for instantiation section.)

		// (BEGIN original - unchanged)
		...
		if (mustCalc)
		{
			// (Calculation logic preserved unmodified.)
...
namespace 
{
	typedef boost::mpl::vector<Float64, Float32, UInt32, UInt64> DistTypeList;
