...
ScalarType SelectRow(ConstPointIter polyBegin, ConstPointIter polyEnd, DensityType measure)
{
	...
	DBG_TRACE(("measure {}", AsString(measure).c_str()));
	...
	if (polySize)
	{
		...
		for (; currPointIndexPtr != lastPointIndexPtr; ++currPointIndexPtr)
		{
			...
			DBG_TRACE(("currPoint  {}", AsString(polyBegin[*currPointIndexPtr]).c_str()));
			DBG_TRACE(("currWidth  {}", AsString(currWidth).c_str()));
			DBG_TRACE(("deltaDArea {}", AsString(deltaDoubleArea).c_str()));
			...
			if (newDoubleAreaSofar >= measure)
			{
				...
				DBG_TRACE(("measure   {}", AsString(measure).c_str()));
				DBG_TRACE(("currWidth {}", AsString(currWidth).c_str()));
				DBG_TRACE(("prevWidth {}", AsString(prevWidth).c_str()));
				DBG_TRACE(("currRow   {}", AsString(currRow).c_str()));
				DBG_TRACE(("prevRow   {}", AsString(prevRow).c_str()));
				...
				DBG_TRACE(("currRow'  {}", AsString(currRow).c_str()));
				DBG_TRACE(("tmp1 {}", AsString(tmp1).c_str()));
				...
				DBG_TRACE(("row {}", AsString(row).c_str()));
