...
void DataArrayBase<V>::DoSimplifyCheckMode(DataCheckMode& dcm) const
{
	...
	{
		if (dcm == DCM_CheckBoth && !ContainsUndefined(GetValueRangeData()->GetRange()))
...
auto CreateAbstrHeapTileFunctor(const AbstrDataItem* adi, const bool mustClear MG_DEBUG_ALLOCATOR_SRC_ARG) -> std::unique_ptr<AbstrDataObject>
{
	...
	if (currTRD->GetNrTiles() > 1 && !adi->IsCacheItem())
		reportF(ST_MinorTrace, "CreateAbstrHeapTileFunctor(attribute<%s> %s(%d tiles), ", adi->GetAbstrValuesUnit()->GetValueType()->GetName(), adi->GetFullName().c_str(), currTRD->GetNrTiles());
