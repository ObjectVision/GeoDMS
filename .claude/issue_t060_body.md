...
Regression test **t060** (`Storage_BAGSnapshot_Utrecht_GeoPackage_Compare`) generates `snapshot_Utrecht_20210701.gpkg`. After the v19â†’v20 migration of `polygon_i4HV(geom, sz)` â†’ `geos_buffer_multi_polygon(geom, sz, 16b)` (boost::polygon Minkowski â†’ GEOS), **35.76 % of rows in the `adres` layer have collapsed to coordinates `(-0.001, -0.001)`** instead of receiving geometry from their backing entity (vbo / ligplaats / standplaats / nevenadres).

Affected: **407 301 / 1 138 872** `adres` rows â€” exactly the secondary-addresses without a directly-matching verblijfsobject (`vbo` count = 731 571 = the OK count).
...
**Perfect correlation** â€” every bad row has `woonplaatsNaam = NULL`, every ok row has a value (Utrecht / Amersfoort / Nieuwegein / Veenendaal / Zeist â€¦).

Sample bad rows (FID 1â€“10, all real BAG-registered addresses on Atalanta street, Zeewolde â€” postcode 3892EJ/ED/EE; should be at RD â‰ˆ (190 k, 482 k)):
...
The `polygon_i4HV` â†’ `geos_buffer_multi_polygon` migration is correct for the primary-vbo path (geometry resolves cleanly there: 731 571 OK rows = vbo count). But the secondary-address fallback chain â€” `nummeraanduiding` â†’ (no vbo) â†’ inherit from ligplaats / standplaats / nevenadres â€” relied on the boost::polygon path's ipoint-typed multipolygon flowing through the join. The new GEOS-based dpoint path doesn't propagate via the same fallback, and the join produces a default-zero geometry instead of either inheriting or erroring.
...
1. Build cmake-Release 20.0.0 (or the equivalent msbuild flavor) with the v19â†’v20 cfg migrations applied.
...
- Cfg with the polygon_i4HV call site (now migrated): `prj_snapshots/BAG20/cfg/BAG20_MakeSnaphot/MakeSnapshot{,_gpkg}/afleidingen.dms` line 126:
  ...
  ```
- Migration script that performed the rename (one-liner, no other changes): `tst migration commit 4aa30ab â€” v19â†’v20 migration: operator renames + multi-build-target runner`.

## Suggested fixes (in increasing scope)

1. Make the fallback chain in BAG20 cfg explicit: use `coalesce` (or DMS-equivalent) so `adres` rows without a `vbo` join take geometry from `nummeraanduiding` â†’ `ligplaats`/`standplaats` directly, not from a Minkowski-buffered side-product.
2. Review `geos_buffer_multi_polygon`'s contract for "no-op / pass-through" cases â€” `polygon_i4HV(0d)` â‰¡ identity, so the migrated `geos_buffer_multi_polygon(g, 0, 16b)` should equally be identity. If GEOS empties the geometry on zero-buffer or NULL input, it should propagate NULL rather than a degenerate origin point.
