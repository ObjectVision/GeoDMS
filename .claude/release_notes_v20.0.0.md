First public 20.0.0 pre-release with three installer flavors: Windows MSBuild (`.m`), Windows CMake (`.c`), and Linux/WSL (`.l` `.deb` + `.tar.gz`). Same source, three build paths — pick whichever matches your workflow.
...
Linux-only verification of the signed `.tar.gz` is documented in [`nsi/VERIFY-LINUX.md`](https://github.com/ObjectVision/GeoDMS/blob/v20.0.0/nsi/VERIFY-LINUX.md).
...
### Build & release
- New `BuildSignAndCreateSetupCmake.bat` (`.c`) and `BuildSignAndCreateSetupLinux.bat` (`.l`)
...
**Full changelog**: https://github.com/ObjectVision/GeoDMS/compare/v19.3.0...v20.0.0
