...
FSS write fails to overwrite existing `.dmsdata` files whose name contains non-ASCII characters (e.g. Greek `_β`)

# Body

## Summary

When a configuration writes attributes whose identifier contains a non-ASCII character (e.g. Greek lower-case beta `β`), the GeoDMS engine creates the corresponding `.dmsdata` file fine on the **first** run, but on **subsequent** runs fails with `FileSystem Error: Write permission denied` — *only for the non-ASCII-named files*. ASCII-named sibling attributes in the same `.fss` folder overwrite without issue.
...
and the calculation chain via `Templates/.../Trends/Write_Betas_Objecten` writes attributes called `Objecten`, `Objecten_β`, `BrutoOpp`, `BrutoOpp_β` into:
...
2. **Run** `python full.py -version local` (or `local-msbuild-release`). The test runs to completion, all four `.dmsdata` files are created, including `Objecten_β.dmsdata` and `BrutoOpp_β.dmsdata`. ✅

3. **Run again** — without clearing the folder. The test now fails with:
   ```
   ...
   'C:/LocalData/regression/RSopen_RegressieTest_v2025/BaseData/Vastgoed/
    Verblijfsrecreatie/Provincie/Betas_Objecten_Nederland.fss/Objecten_β.dmsdata' denied
   ```
   The ASCII-named siblings (`Objecten.dmsdata`, `BrutoOpp.dmsdata`) overwrite cleanly. Only the `_β` files fail.
...
$ mv Objecten_β.dmsdata x.tmp && mv x.tmp Objecten_β.dmsdata    # rename round-trip OK
$ dd if=/dev/zero of=Objecten_β.dmsdata bs=52 count=1            # write OK
```
...
Delete the offending `_β.dmsdata` files before re-running. Acceptable for CI but not for interactive use.
