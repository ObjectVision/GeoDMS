...
//		table of ObjectTypes
		std::map<ClassCPtr, UInt32> classNrs;
			...
			AbstrUnit* auTableTypes = Unit<UInt32>::GetStaticClass()->CreateUnit(result, GetTokenID_mt("ObjectTypes")).get();
			{
...
public:
	...
	//	table of StorageManagers
		{
			AbstrUnit* auSmTypes = Unit<UInt32>::GetStaticClass()->CreateUnit(result, GetTokenID_mt("StorageManagers")).get();
