...
bool ForEach_CreateResult(TreeItemDualRef& resultHolder, const ArgSeqType& args, arg_index argCount, TokenID groupNameID, field_spec fs)
{
	...
	for (datarow_id i=0; i!= maxNrIter; ++i)
	{
		...
		{
			assert(!optVuContext);
			iter = resultHolder.GetNew()->CreateItemFromPath(subItemName.c_str()).get();
		}
		...
		if (optExprs)       iter                     ->SetExpr (      AcceptIfNewOrEqual(SharedStr(optExprs      ->GetIndexedValue(optExprsIsParam       ? 0 : i)), iter->GetExprMember(), "calculation rule"));
