...
struct OverlayLayerVisitor : tl::fold_t<typelists::domain_ints, OverlayLayerVisitorBase, UnitVisitorImpl>
{};
...
public:
	...
	virtual void CreateResultCaller(TreeItemDualRef& resultHolder, const ArgRefs& args, LispPtr) const
	{
		...
		auto resAtomicRegions_owner = Unit<ResID>::GetStaticClass()->CreateResultUnit(resultHolder.GetNew()); AbstrUnit* resAtomicRegions = resAtomicRegions_owner.get();
