...
// LabelDrawer is defined in LabelDrawer.h (needed by DrawPolygons.h)
...
bool DrawArcs(const GraphicArcLayer* layer, const FeatureDrawer& fd, const PenIndexCache*  penIndices)
{
	...
	typedef typename SequenceBoundingBoxCache<ScalarType>::RectArrayType RectArrayType;
...
bool FeatureLayer::GetTooltipText(TooltipCollector& ttc) const
{
	...
	if (attrItem)
		ado = debug_cast<const AbstrDataItem*>(attrItem->GetCurrUltimateItem().get())->m_DataObject.get();
	if (!ado)
		return false;
