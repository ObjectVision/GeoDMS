...
void WmsLayer::SetSpecContainer(const TreeItem* specContainer)
{
	...
	m_SpecContainer = make_shared_tree(specContainer, existing_obj{});
	...
	std::shared_ptr<const TreeItem> tileMatrices = make_shared_tree(specContainer->GetConstSubTreeItemByID(GetTokenID_mt("TileMatrix")).get(), existing_obj{});
...
void WmsLayer::Sync(TreeItem* viewContext, ShvSyncMode sm)
{
	...
	if (sm == SM_Load)
	{
		std::shared_ptr<const TreeItem> specContainer;
