      ...
      DP_ValueInfo:
        begin
          ...
          if actionStr = 'GRID' then
            action.IsReady := ViewDetails(wbMetaData.DefaultInterface, ti, dmfGeneral.AdminMode, dp, 2, 0, action.Row, action.Col, action.ExtraInfo)
          else if actionStr = 'ATTR' then
            action.IsReady := ViewDetails(wbMetaData.DefaultInterface, ti, dmfGeneral.AdminMode, dp, 1, action.RecNo, 0, 0, action.ExtraInfo);
        end
    end;
  finally
    bActive := false;
    if dp <> DP_ValueInfo then SetStatisticsInterest(nil);
  end
end;
